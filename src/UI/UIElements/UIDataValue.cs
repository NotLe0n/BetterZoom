...
namespace BetterZoom.src.UI.UIElements
{
    ...
    abstract class UIRangedDataValue<T> : UIDataValue<T>
    {
        ...
        public UIRangedDataValue(string label = "", T defaultValue = default, T min = default, T max = default, bool enforceMin = false, bool enforceMax = false) : base(label, defaultValue)
        {
            this.min = min;
            this.max = max;
